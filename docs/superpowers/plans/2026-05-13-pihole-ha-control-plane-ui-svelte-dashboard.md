...
**Execution status (2026-05-14):** Tasks **1–5** are **implemented** in this repo under **`apps/ui/`** (Zod + gateway + Vite entry + operator shell + DnD grid + log panel + dev proxy). **Task 6** (multi-stage Dockerfile build inside **`pihole-ha`**) remains **optional**; shipping uses **`pihole-ha`** **`scripts/sync-pihole-cp-ui.sh`** and **`scripts/push-cp-ui-to-pi2.sh`** instead. **Phase 4** (aggregation) is **out of scope** here — see **`2026-05-13-pihole-ha-control-plane-phase-4-aggregation-optional.md`**.
