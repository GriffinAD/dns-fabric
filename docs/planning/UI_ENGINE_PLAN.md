...
| **P2** | **Done** | `DataTableTile` (DHCP pools/clients/reservations), `PerfMetricTile` for metric tiles, `pluginGridPolicy.ts` (no `pluginId ===` in `gridPlacement` / `builtinMeta`), `alignPerfGridAlignment.ts`, `tileOptionsZod` under `lib/plugins/`, `PluginTileMount` Zod gate + `resolvePluginTileSettings` / `perf/PerfOptionsForm.svelte`, **blocking** `ENFORCE_UI_PLUGIN_GUARD=1` + dashboard import guard in CI. Thin `perf/index.ts` folder indexes optional; Playwright `reuseExistingServer` only with `PW_REUSE_DEV_SERVER=1` so `VITE_E2E_THROWING` applies. |
| **P3** | **Done** | **`layoutStore.ts`:** debounced PUT, flush, cache/server source, `localPersistBlocked` / reason when stored layout version &gt; 2; gate cleared on `acceptServerLayout`, successful `resetToBaseline`, or `clearStoredDashboardLayoutAndUnlock`. **`layoutStorage`:** `saveDashboardLayout` no-ops while gate is set; unlock helper clears storage. **`tileOptionsZod.ts`:** `perf.*` vs non-perf options schemas; `layoutZod` validates tile `options` per plugin id (no `registry` import to avoid cycles). **`DataGateway`:** Zod on GET/POST + layout; SSE `fabricEventSchema`. **`migrations/stripLegacyPerfSummary.ts`:** P3.6. **`App.svelte`:** banner + reload when local persist blocked. **Stretch / later:** wire `optionsSchema` from `registry` if a single map grows unwieldy; Flowbite toast for PUT failures; App line budget ≤130; richer migrations registry; full `LayoutStore` spec surface (`hintGrid`, etc.). |
