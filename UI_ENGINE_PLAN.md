...
| **P3** | **Partial** | **Landed:** `layoutStore.ts` (debounced PUT, flush, cache/server source, errors). **`DataGateway`:** runtime Zod on GET/POST JSON via `openapiZod` + `layoutZod`; `GatewayError` (`http_error` \| `parse_failed`); SSE payloads validated with `fabricEventSchema`. **`layoutZod.ts`:** layout JSON mirrors `layout.schema.json` + `gridPlacement` refinements; `parseDashboardLayout` delegates to Zod then normalizes. **`migrations/stripLegacyPerfSummary.ts`:** legacy `perf.summary` strip (P3.6). **P3.7:** `layoutJsonUnsupportedVersionMessage` + `loadDashboardLayout` warns and ignores v3+ payloads. Tests: `layoutZod.test.ts`, extended `dataGateway` / `layoutStorage` tests. **Not done:** `optionsSchema` union from plugin registry (P3.5 remainder); richer migrations registry; `hintGrid` / full `LayoutStore` spec surface; Flowbite toast; App line budget ≤130. |
