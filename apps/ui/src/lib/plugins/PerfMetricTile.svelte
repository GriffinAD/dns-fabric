<script lang="ts">
  ...
  let {
    ...
    /** Dashboard grid: colSpan follows gauge count (capped); a single gauge uses 2×1 as an exception. */
  ...
  /** Intratile grid columns + dashboard colSpan (wider when exactly one gauge). */
  const layoutMeta = $derived.by(() => {
    ...
    const dashboardCols = n === 1 ? clampGridColSpan(2) : gaugeCols;
...
<Card size="xl" class="h-full overflow-auto">
  {#snippet children()}
    <div class="p-2 sm:p-3">
      ...
      {:else if !summary}
        <p class="text-center text-xs text-gray-500 dark:text-gray-400">Loading…</p>
      {:else if percentOnly}
        {#if metric === "cpu"}
      ...
      {:else if metric === "cpu"}
        <div
          class="grid w-full gap-x-1 gap-y-2"
          style={gaugeGridStyle}
          data-testid="perf-metric-gauges"
        >
          {#if cpuTotal}
            <SemicircleGauge label="CPU" percent={cpuDisplay} mini miniFillCell={stretchGaugesToCells} />
          {:else}
            {#each summary.cpu_core_percent ?? [cpuDisplay] as pct, i (i)}
              <SemicircleGauge label="Core {i}" percent={pct} mini miniFillCell={stretchGaugesToCells} />
            {/each}
          {/if}
        </div>
      {:else if metric === "ram"}
        ...
        >
          {#if byVolume && summary.disk_volumes?.length}
            {#each summary.disk_volumes as v, i (`${i}-${v.label}`)}
              <SemicircleGauge label={v.label} percent={v.used_percent} mini miniFillCell={stretchGaugesToCells} />
            {/each}
          {:else}
            <SemicircleGauge label="Disk" percent={summary.disk_used_percent ?? 0} mini miniFillCell={stretchGaugesToCells} />
          {/if}
        </div>
      {/if}
    </div>
