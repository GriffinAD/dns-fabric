...
describe("DataGateway", () => {
  ...
  it("setKeaFabricApiBaseUrl falls back to VITE_API_BASE_URL when Kea env is unset", async () => {
    vi.unstubAllEnvs();
    vi.stubEnv("VITE_API_BASE_URL", "https://api-only.example/");
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ok: true,
    ...
    const gw = new DataGateway("");
    gw.setKeaFabricApiBaseUrl(null);
    await gw.getMeta();
    expect(fetch).toHaveBeenCalledWith("https://api-only.example/api/v1/meta", { headers: {} });
  });
