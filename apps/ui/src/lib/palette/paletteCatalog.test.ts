...
describe("buildPaletteCatalog", () => {
  ...
  it("sorts by label within the same derived category", () => {
    ...
    expect(items[3]?.kind === "plugin" && items[3].id).toBe("dhcp.a");
    expect(items[4]?.kind === "plugin" && items[4].id).toBe("dhcp.z");
  });

  it("places core layout chips first and sorts plugins", () => {
    const items = buildPaletteCatalog([
      { id: "z.last", name: "Z", enabled: true },
      { id: "a.first", name: "A", enabled: true },
      { id: "off", name: "Off", enabled: false },
    ]);
    expect(items.slice(0, 3).every((i) => i.kind === "core")).toBe(true);
